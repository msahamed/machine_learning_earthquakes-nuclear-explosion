...
model_name = ['energy', 'variable', 'deviatoric', 'fixed']
...
for i in xrange(num_of_models):
    for j in range(1, number_of_files):
        ...
        df.to_csv('stress_data.csv', mode='a', index=False, header=False)
        print("Finished appending data of : " + str(model_name[i]) + "_timestep_" + str(j))

data = pd.read_csv('stress_data.csv')
shuffled_data = data.sample(frac=1)
shuffled_data.to_csv('stress_data_shuffled.csv', mode='a', index=False, header=False)
